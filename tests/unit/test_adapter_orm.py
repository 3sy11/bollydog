...
from bollydog.adapters.rdb import SqlAlchemyProtocol, SqlAlchemyAsyncUnitOfWork, map_imperatively, DatabasesUnitOfWork
...
async def test_adapter_rdb_sqlalchemy_unit_of_work():
    unit_of_work = SqlAlchemyAsyncUnitOfWork(url='sqlite+aiosqlite:///:memory:', metadata=R.metadata)
...
#
# @pytest.mark.asyncio
# async def test_adapter_rdb_databases_unit_of_work():
#     unit_of_work = DatabasesUnitOfWork(url='sqlite+aiosqlite:///:memory:', metadata=R.metadata)
#
#     async with unit_of_work:
#         await unit_of_work.create_all()
#         protocol = SqlAlchemyProtocol(unit_of_work=unit_of_work)
