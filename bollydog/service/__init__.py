...
def build_services(parsed: dict, mode: str = 'service') -> dict:
    """Instantiate services from parsed config dict, return apps registry.
    ...
    from bollydog.globals import _apps_ctx_stack

    apps = {}

    def _register(svc):
        key = f'{svc.domain}.{svc.alias}'
        apps[key] = svc
    ...
    if mode == 'execute':
        for node_name, node_conf in parsed.items():
            ...
            _register(smart_import(module).create_from(**node_conf))
    else:
        for node_name, node_conf in parsed.items():
            node_conf = dict(node_conf)
            module = node_conf.pop('module', node_name)
        ...
        if os.getenv('ENTRYPOINT_HTTP_ENABLED', '0') == '1': _register(HttpService.create_from())
        if os.getenv('ENTRYPOINT_WS_ENABLED', '0') == '1': _register(SocketService.create_from())
        if os.getenv('ENTRYPOINT_UDS_ENABLED', '0') == '1': _register(UdsService.create_from())

    _apps_ctx_stack.push_without_automatic_cleanup(apps)

    for svc in list(apps.values()):
        resolved = []
        for dep_key in (svc.depends if isinstance(svc.depends, (list, tuple)) and svc.depends and isinstance(svc.depends[0], str) else []):
            dep = apps.get(dep_key)
