...
class AppHandler:
    messages: Dict[str, Type[BaseCommand]] = {}
    commands: Dict[Type[BaseCommand], "AppHandler"] = {}
    ...
    async def __call__(self, message: BaseCommand) -> Any:
    ...
    async def _handle_async_function(self, message):
        ...
        if isinstance(next_message, BaseCommand):
    ...
    async def _handle_async_generator(self, message):
        ...
        try:
            while True:
                ...
                if not isinstance(next_message, BaseCommand):
                    return next_message
    ...
    @property
    def call(self) -> Callable[[BaseCommand], Awaitable[BaseCommand]]:
        return hub.put_message if hub.state == "running" and _message_ctx_stack.top.qos == 0 else hub.execute

    @classmethod
    def get_message_handlers(cls, message: Type[BaseCommand]) -> List["AppHandler"]:
